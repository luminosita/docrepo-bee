...
func TestPutDocumentGoodRequest(t *testing.T) {
	...
	repoRequest := &documents.PutDocumentRepositorerRequest{Name: "laza", Size: 1234, Reader: reader}
	repoResponse := &documents.PutDocumentRepositorerResponse{DocumentId: "pera"}

	m.pdr.EXPECT().PutDocument(gomock.Eq(repoRequest)).Return(repoResponse, nil)

	res, err := d.Execute(&documents2.PutDocumenterRequest{Name: "laza", Size: 1234, Reader: reader})
